...
// {
//   Eigen::Vector3i idx;
//   idx(0) = std::min(std::max(int((pt(0) - x_l_) * inv_resolution_), 0),
//   GLX_SIZE_ - 1); idx(1) = std::min(std::max(int((pt(1) - y_l_) *
//   inv_resolution_), 0), GLY_SIZE_ - 1); idx(2) = std::min(std::max(int((pt(2)
//   - z_l_) * inv_resolution_), 0), GLZ_SIZE_ - 1); return idx;
// };
...
void renderSensedPoints(const ros::TimerEvent& event) {
  ...
  pcl::PointXYZ pt;
  if (kdtreeLocalMap_.radiusSearch(searchPoint, sensing_horizon_,
    ...
    for (size_t i = 0; i < pointIdxRadiusSearch_.size(); i++) {
      ...
      // if ((fabs(pt.z - odom_.pose.pose.position.z) / (pt.x -
      // odom_.pose.pose.position.x)) > tan(M_PI / 12.0))
      //   continue;
...
int main(int argc, char** argv) {
  ...
  dynamic_sub_ =
      nh.subscribe("/move_base_simple/goal", 10, dynamicCloudCallBack);
