...
//   state.motor_rpm = Eigen::Array4d(hover_rpm, hover_rpm, hover_rpm,
//   hover_rpm); quad.setState(state);
...
//     state.omega(1) << ", " << state.omega(2) << ", " << state.motor_rpm(0) <<
//     std::endl;

//     clock_gettime(CLOCK_MONOTONIC, &ts2);
//     time_taken += ((ts2.tv_sec - ts1.tv_sec) * 1000000000UL + (ts2.tv_nsec -
//     ts1.tv_nsec)); int64_t time_sleep = i * dt / 2 * 1e9 - time_taken;
//     clock_gettime(CLOCK_MONOTONIC, &ts1);
//     if (time_sleep > 0)
//     {
...
//   std::cerr << "Time: " << (ts_end.tv_sec - ts_start.tv_sec) * 1e6 +
//   (ts_end.tv_nsec - ts_start.tv_nsec) / 1e3 << " usec" << std::endl;
