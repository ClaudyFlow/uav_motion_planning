...
void Quadrotor::step(double dt) {
  ...
  for (int i = 0; i < 22; i++) {
    if (std::isnan(internal_state_[i])) {
      std::cout << "dump " << i << " << pos ";
      for (int j = 0; j < 22; j++) std::cout << save[j] << " ";
...
void Quadrotor::operator()(const Quadrotor::InternalState& x,
  ...
  for (int i = 0; i < 4; i++) cur_state.motor_rpm(i) = x[18 + i];
  ...
  vnorm = cur_state.v;
  if (vnorm.norm() != 0) vnorm.normalize();
  ...
  for (int i = 0; i < 4; i++) dxdt[18 + i] = motor_rpm_dot(i);

  for (int i = 0; i < 22; i++)
    if (std::isnan(dxdt[i])) dxdt[i] = 0;
}
