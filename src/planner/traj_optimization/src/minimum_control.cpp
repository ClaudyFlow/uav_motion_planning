...
void MinimumControl::getGradient() { q_.setZero(); }
...
void MinimumControl::getConstraintMatrix(Eigen::VectorXd& time_vec) {
  ...
  A_.insert(3 + 4 * (time_vec.size() - 1), 6 * (time_vec.size() - 1) + 1) =
      time_vec[time_vec.size() - 1];
  A_.insert(3 + 4 * (time_vec.size() - 1), 6 * (time_vec.size() - 1) + 2) =
      pow(time_vec[time_vec.size() - 1], 2);
  A_.insert(3 + 4 * (time_vec.size() - 1), 6 * (time_vec.size() - 1) + 3) =
      pow(time_vec[time_vec.size() - 1], 3);
  A_.insert(3 + 4 * (time_vec.size() - 1), 6 * (time_vec.size() - 1) + 4) =
      pow(time_vec[time_vec.size() - 1], 4);
  A_.insert(3 + 4 * (time_vec.size() - 1), 6 * (time_vec.size() - 1) + 5) =
      pow(time_vec[time_vec.size() - 1], 5);

  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1)) = 0.0;
  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1) + 1) = 1.0;
  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1) + 2) =
      2 * time_vec[time_vec.size() - 1];
  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1) + 3) =
      3 * pow(time_vec[time_vec.size() - 1], 2);
  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1) + 4) =
      4 * pow(time_vec[time_vec.size() - 1], 3);
  A_.insert(4 * (time_vec.size()), 6 * (time_vec.size() - 1) + 5) =
      5 * pow(time_vec[time_vec.size() - 1], 4);
  ...
  A_.insert(4 * (time_vec.size()) + 1, 6 * (time_vec.size() - 1) + 3) =
      6 * time_vec[time_vec.size() - 1];
  A_.insert(4 * (time_vec.size()) + 1, 6 * (time_vec.size() - 1) + 4) =
      12 * pow(time_vec[time_vec.size() - 1], 2);
  A_.insert(4 * (time_vec.size()) + 1, 6 * (time_vec.size() - 1) + 5) =
      20 * pow(time_vec[time_vec.size() - 1], 3);
}
...
bool MinimumControl::solve(Eigen::VectorXd& pos_1d, Eigen::Vector2d& bound_vel,
                           Eigen::Vector2d& bound_acc,
...
void MinimumControl::reset() { coef_1d_.setZero(); }

Eigen::VectorXd MinimumControl::getCoef1d() { return coef_1d_; }
