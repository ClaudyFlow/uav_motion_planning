...
void RRT::setGridMap(GridMap::Ptr& grid_map) { this->grid_map_ = grid_map; }
...
Eigen::Vector3d RRT::Step(Eigen::Vector3d from, Eigen::Vector3d to,
                          double step_length) {
  Eigen::Vector3d step_dir = to - from;
  if (step_dir.norm() <= step_length) return to;
  step_dir.normalize();
...
bool RRT::isCollisionFree(Eigen::Vector3d from, Eigen::Vector3d to,
